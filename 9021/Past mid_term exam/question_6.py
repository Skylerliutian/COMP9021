...
def f(for_seed, n, upper_bound):
    ...
    for i in range(n):
        for j in range(n):
            all_elements.append(square[i][j])
    all_elements = sorted(all_elements)
    for i in range(n):
        t = []
        for j in range(i * n, (i + 1) * n):
            t.append(all_elements[j])
        reverse_order.append(t)
    ordered_square = zip(*reverse_order)
    print(ordered_square)

    # ordered_square = deepcopy(reverse_order)
    # for i in range(n):
    #     for j in range(n):
    #         ordered_square[i][j] = reverse_order[j][i]
    if duplicates:
