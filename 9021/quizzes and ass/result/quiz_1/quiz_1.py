...
def describe_automaton(transitions):
    ...
    for key in transitions:
        ...
        print('When in state \"',key[0],'\" and processing \"',key[1],'\", automaton\'s state becomes \"',x,'\".',sep='')
...
('state_2', 0): 'state_1', ('state_2', 1): 'state_2'}
    '''
    transitions = {}
    for l in transitions_as_list:
       a, b = l.split(':')
       y1,y2 = a.split(',')
       transitions[(y1,int(y2))] = b 
    return transitions
...
def accepts(transitions, word, initial_state, accept_state):
    ...
    for wd in word:
        ...
        if (current_state, int(wd)) in transitions.keys():
            current_state = transitions[(current_state, int(wd))]
        else:
            return False
            break
